...
class FeatureEncoder:
    ...
    def transform(self, current: dict, history: Sequence[dict], positions=None) -> np.ndarray:
        ...
        values = {
            ...
            "temperature_c": item.temperature_c if item.temperature_c is not None else 25.0,
        ...
        return np.asarray([[float(values[name] if values[name] is not None else 0.0) for name in FEATURE_NAMES]], dtype=float)
