...
def health_for_node(features: NodeFeatures, *, stale: bool = False) -> HealthResult:
    reasons: list[str] = []
    if features.flags is not None and (features.flags & 0x0F):
        reasons.append("protocol fault or calibration flag")
    if features.battery_mv is not None and features.battery_mv < 3500:
        reasons.append("low battery")
    if ((features.rssi_dbm is not None and features.rssi_dbm < -115)
            or (features.snr_db is not None and features.snr_db < -5)):
        reasons.append("degraded communication")
    if features.gnss_fix is not None and features.gnss_fix == 0:
    ...
    status = (
        SensorHealth.FAULTY
        if (features.flags is not None and (features.flags & 0x07))
        else SensorHealth.SUSPECT if reasons else SensorHealth.HEALTHY
    )
