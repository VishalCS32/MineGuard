...
from security.anti_theft import evaluate_anti_theft_frame
...
def compute_node_baseline(history_records: Sequence[Any]) -> tuple[float, float]:
    ...
    for rec in history_records:
        ...
        bat = rec.battery_mv if hasattr(rec, "battery_mv") else rec.get("battery_mv")
        flags = rec.flags if hasattr(rec, "flags") else rec.get("flags")
        bat_ok = (bat is None) or (bat >= 3300.0)
        flags_ok = (flags is None) or (flags == 0)
        if bat_ok and flags_ok and float(tilt) < 45.0:
...
def predict(current: Sequence[Mapping[str, Any]],
            ...
            *, warning_tilt_deg: float = 0.5,
            critical_tilt_deg: float = 1.0,
            registered_positions: Mapping[str, Any] | None = None,
            anti_theft_tracker: Any = None) -> dict[str, Any]:
    ...
    if not current:
        return {
            ...
            "data_sufficiency": {
                ...
                "available_prior_observations": 0,
            ...
            "time_to_threshold": {
                ...
                "warning_status": "INSUFFICIENT_HISTORY",
                "warning_hours": None,
                "critical_threshold_deg": critical_tilt_deg,
                "critical_status": "INSUFFICIENT_HISTORY",
                "critical_hours": None,
                "unit": "hours",
            },
            "spatial": {
            ...
            "physics": {
                "residual_score": None,
                "status": "UNAVAILABLE",
                "explanation": "No reporting telemetry nodes in the current frame.",
            },
            "sensor_health": {
                "status": "HEALTHY",
                "healthy_nodes": [],
            ...
            "nodes": [],
            "dominant_factors": [],
            "explanation": "No telemetry nodes reported in the current frame.",
        }

    if history is not None and not isinstance(history, Mapping):
        raise TypeError("history must be a mapping of node_id to observations")
    if derived is not None and not isinstance(derived, Mapping):
        raise TypeError("derived must be a mapping of node_id to physics parameters")

    nodes = tuple(NodeTelemetry.from_mapping(item) for item in current)
    seen_ids = set()
    for n in nodes:
        if n.node_id in seen_ids:
            raise ValueError(f"duplicate node_id in telemetry frame: {n.node_id}")
        seen_ids.add(n.node_id)
    history_records = {
        str(node_id): tuple(sorted((NodeTelemetry.from_mapping(item) for item in records), key=lambda x: x.timestamp))
    ...
    is_ood, ood_warnings = _check_data_quality(frame, nodes)
    ...
    anti_theft_summary = evaluate_anti_theft_frame(
        nodes,
        history_records=history_records,
        registered_positions=registered_positions,
        tracker=anti_theft_tracker,
    )
    theft_by_node = {e.node_id: e.to_dict() for e in anti_theft_summary.events}
    ...
    for item in frame.nodes:
        ...
        if pos_self and pos_self[0] is not None and pos_self[1] is not None:
            for other_id in elevated_node_ids:
                if other_id != item.node_id:
                    pos_other = node_coords.get(other_id)
                    if (pos_other and pos_other[0] is not None and pos_other[1] is not None
                            and hypot(pos_self[0] - pos_other[0], pos_self[1] - pos_other[1]) <= 100.0):
                        spatial_support = True
                        break
        has_coords = bool(pos_self and pos_self[0] is not None and pos_self[1] is not None)
        if not spatial_support and has_coords and (frame.spatial_consistency >= 0.5 and len(frame.affected_node_ids) > 1):
            spatial_support = True
        ...
        derived_for_node = None
        if derived and item.node_id in derived:
            d_item = derived[item.node_id]
            if isinstance(d_item, Mapping):
                derived_for_node = dict(d_item)
                obs = d_item.get("observed")
                exp = d_item.get("expected")
                if obs is None and "subsidence_mm" in d_item:
                    obs = d_item.get("subsidence_mm")
                    exp = d_item.get("expected_subsidence_mm")
                if obs is None and "strain_mm_per_m" in d_item:
                    obs = d_item.get("strain_mm_per_m")
                    exp = d_item.get("expected_strain_mm_per_m")
                if obs is not None and exp is not None:
                    res = abs(float(obs) - float(exp))
        ...
        node_ood, node_ood_warnings = _check_node_quality(item, node_map.get(item.node_id))
        ...
        if is_sensor_fault:
            reason_codes.append("SENSOR_FAULT")
        ...
        if sufficient and robust_rate <= 0.001 and tilt < warning_tilt_deg:
            reason_codes.append("NO_POSITIVE_TREND")
        ...
        node_outputs.append({
            ...
            "anti_theft": theft_by_node.get(item.node_id),
            "health": {
            ...
            "deformation": {
                ...
                **({"derived": derived_for_node} if derived_for_node is not None else {}),
            },
            "risk": {
    ...
    alarm = bool(any_confirmed or (not is_ood and (risk_score >= 0.60 or max_node_risk >= 0.60)))
    ...
    elif not is_ood and risk_score >= 0.60:
    ...
    if overall_status in {"NORMAL", "WARNING", "CRITICAL", "SENSOR_FAULT"}:
        overall_severity = overall_status
    elif overall_status == "INSUFFICIENT_HISTORY":
    ...
    # Contextual factors & explainable early-warning reasoning
    any_vibration_only = any(n["anomaly"]["evidence_status"] == "OPERATIONAL_VIBRATION" for n in node_outputs)
    any_cooling = any(n["anomaly"]["evidence_status"] == "COOLING_NORMAL" for n in node_outputs)

    factors = []
    if not sufficient or progression == DeformationState.INSUFFICIENT_HISTORY:
    ...
    if any_confirmed:
        confirmed_nodes = [n for n in node_outputs if n["deformation"]["status"] == "CONFIRMED_PHYSICAL"]
        multi_node = len(confirmed_nodes) > 1
        has_spatial = any(n["deformation"]["evidence"]["spatial"] for n in confirmed_nodes)
        nodes_str = "across multiple nodes" if multi_node else f"on node {confirmed_nodes[0]['node_id']}"
        spatial_str = " with spatial corroboration" if has_spatial else ""
        desc = f"Persistent positive deformation detected {nodes_str}{spatial_str}."
        if top_w_hours is not None:
            desc += f" Warning threshold estimated in {top_w_hours:.1f} hours."
        elif top_w_status == "ALREADY_EXCEEDED":
            desc += " Warning threshold is already exceeded."
        elif top_c_hours is not None:
            desc += f" Critical threshold estimated in {top_c_hours:.1f} hours."
        elif top_c_status == "ALREADY_EXCEEDED":
            desc += " Critical threshold is already exceeded."
        factors.append(desc)
    elif any_fault:
        factors.append("Telemetry indicates sensor fault; physical deformation alarm suppressed.")
    elif any_vibration_only:
        factors.append("Elevated vibration detected without persistent deformation or spatial corroboration; classified as operational vibration.")
    elif any_cooling:
        factors.append("Thermal cooling/settling detected near baseline; normal behavior.")

    if any(item.status.value not in {"HEALTHY", "SUSPECT"} for item in all_health):
    ...
    return {
        ...
        "anti_theft": anti_theft_summary.to_dict(),
        "sensor_health": {
        ...
        "explanation": " ".join(factors) if factors else "No credible persistent physical deformation detected.",
    }
...
def _check_node_quality(item, node=None) -> tuple[bool, list[str]]:
    warnings: list[str] = []
    if item.tilt_deg > 75.0:
        warnings.append(f"Node {item.node_id} tilt ({item.tilt_deg:.1f} deg) exceeds physical operational bounds.")
    if item.temperature_c is not None and (item.temperature_c < -25.0 or item.temperature_c > 75.0):
        warnings.append(f"Node {item.node_id} temperature ({item.temperature_c:.1f} C) is out of expected mine range.")
    if item.battery_mv is not None and (item.battery_mv < 2400 or item.battery_mv > 4600):
    ...
    if node is not None:
        ...
        if node.timestamp > now_utc + timedelta(days=1):
            warnings.append(f"Node {node.node_id} timestamp ({node.timestamp.isoformat()}) is in the future.")
    return bool(warnings), warnings


def _check_data_quality(frame, nodes=()) -> tuple[bool, list[str]]:
    warnings: list[str] = []
    node_map = {n.node_id: n for n in nodes}
    for item in frame.nodes:
        _, item_warnings = _check_node_quality(item, node_map.get(item.node_id))
...
def _physics_residual(derived) -> float | None:
    ...
    for item in derived.values():
        ...
        if obs is None and "subsidence_mm" in item:
            obs = item.get("subsidence_mm")
            exp = item.get("expected_subsidence_mm")
        if obs is None and "strain_mm_per_m" in item:
            obs = item.get("strain_mm_per_m")
            exp = item.get("expected_strain_mm_per_m")
        if obs is not None and exp is not None:
            o = float(obs)
            e = float(exp)
            if 0.0 <= o <= 1.0 and 0.0 <= e <= 1.0:
                residuals.append(abs(o - e))
            else:
                # Millimeter scale: scale error relative to 100.0 mm reference scale
                residuals.append(abs(o - e) / 100.0)
...
def _physics_result(derived) -> dict[str, Any]:
    ...
    has_reconstructed = any(
        isinstance(v, Mapping) and ("subsidence_mm" in v or "strain_mm_per_m" in v)
    ...
    if has_reconstructed:
        return {
            "residual_score": None,
            "status": "UNAVAILABLE",
            "explanation": "Backend supplied array reconstruction (subsidence/strain), but no reference Knothe expected profile was provided for residual computation.",
        }
...
def _record_from_node(node):
    ...
    if getattr(node, "latitude", None) is not None:
        rec["latitude"] = node.latitude
    if getattr(node, "longitude", None) is not None:
        rec["longitude"] = node.longitude
    if getattr(node, "altitude_m", None) is not None:
        rec["altitude_m"] = node.altitude_m
    if getattr(node, "accel_x", None) is not None:
        rec["accel_x"] = node.accel_x
    if getattr(node, "accel_y", None) is not None:
        rec["accel_y"] = node.accel_y
    if getattr(node, "accel_z", None) is not None:
        rec["accel_z"] = node.accel_z
    if getattr(node, "gyro_x", None) is not None:
        rec["gyro_x"] = node.gyro_x
    if getattr(node, "gyro_y", None) is not None:
        rec["gyro_y"] = node.gyro_y
    if getattr(node, "gyro_z", None) is not None:
        rec["gyro_z"] = node.gyro_z
    return rec
