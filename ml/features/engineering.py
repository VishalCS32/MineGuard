...
class NodeFeatures:
    ...
    battery_mv: float | None
    rssi_dbm: float | None
    snr_db: float | None
    gnss_fix: int | None
    gnss_satellites: int | None
    flags: int | None
    history_count: int
    ...
    # --- Preserved Spatial Metadata (Phase 1 & 6) ---
    latitude: float | None = None
    longitude: float | None = None
    altitude_m: float | None = None
    # --- Accelerometer-Derived Physical Features (Phase 3: Optional, SI m/s^2) ---
    accel_magnitude_mps2: float | None = None
    accel_dynamic_dev_mps2: float | None = None
    accel_horizontal_mps2: float | None = None
    accel_vertical_mps2: float | None = None
...
def build_features(current: Sequence[NodeTelemetry],
    ...
    for node in current:
        ...
        # Accelerometer-derived features (Phase 3 - Optional, non-breaking SI m/s^2)
        if node.accel_x is not None and node.accel_y is not None and node.accel_z is not None:
            acc_mag = hypot(hypot(node.accel_x, node.accel_y), node.accel_z)
            acc_dev = abs(acc_mag - 9.80665)
            acc_horiz = hypot(node.accel_x, node.accel_y)
            acc_vert = node.accel_z
        else:
            acc_mag = None
            acc_dev = None
            acc_horiz = None
        ...
        output.append(NodeFeatures(
            ...
            snr_db=node.snr_db,
            gnss_fix=(node.gnss_status & 0x03) if node.gnss_status is not None else None,
            gnss_satellites=((node.gnss_status >> 2) & 0x3F) if node.gnss_status is not None else None,
            flags=node.flags,
            ...
            neighbour_ids=tuple(other.node_id for other in neighbours),
            latitude=node.latitude,
            longitude=node.longitude,
            altitude_m=node.altitude_m,
            accel_magnitude_mps2=acc_mag,
            accel_dynamic_dev_mps2=acc_dev,
            accel_horizontal_mps2=acc_horiz,
            accel_vertical_mps2=acc_vert,
        ))
